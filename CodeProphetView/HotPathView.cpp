// HotPathView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CHotPathView, CColumnTreeView)

CHotPathView::CHotPathView()
{
...
BEGIN_MESSAGE_MAP(CHotPathView, CColumnTreeView)
	ON_NOTIFY(TVN_SELCHANGED, TreeID, OnTvnSelchanged)
	ON_MESSAGE(WM_SETVIEWID, &CHotPathView::OnSetViewID)
END_MESSAGE_MAP()

// CHotPathView drawing

void CHotPathView::OnDraw(CDC* pDC)
{
	CDocument* pDoc = GetDocument();
	// TODO: add draw code here
}

void CHotPathView::OnDetailLevel()
{

}

void CHotPathView::OnTvnSelchanged(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CHotPathView::OnInitialUpdate()
{
	...
	while(numchild--)
		if( calltree->GetNode0()->children.at(numchild)->tot_pc > 5)
			AddFunctions(calltree->GetNode0()->children.at(numchild), TVI_ROOT);

}

void CHotPathView::AddFunctions(TreeNode* thisnode, HTREEITEM level)
{
	CTreeCtrl& tree = GetTreeCtrl();
	CString item;
	...
	while(numchild--)
		if( thisnode->children.at(numchild)->tot_pc > 5 )
			AddFunctions(thisnode->children.at(numchild), newlevel);
}


LRESULT CHotPathView::OnSetViewID(WPARAM wParam, LPARAM lParam)
{
	mythread = lParam;
...
inline CCodeProphetViewDoc* CHotPathView::GetDocument() const
{
...
// CHotPathView diagnostics

#ifdef _DEBUG
void CHotPathView::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CHotPathView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif

CCodeProphetViewDoc* CHotPathView::GetDocument() const // non-debug version is inline
{
