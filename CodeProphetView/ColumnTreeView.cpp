...
BOOL CColumnTreeView::OnEraseBkgnd(CDC* dc)
{
	return TRUE;
}
	...
	case CDDS_ITEMPOSTPAINT:
		{
			...
			if (crTextBk != crWnd)	// draw label's background
			{
				...
				Gdiplus::Color clrSel( 255, GetRValue(crHi), GetGValue(crHi), GetBValue(crHi) );

				// Define a simple brush with this color...
				Gdiplus::Rect rGPItem(rcLabel.left, rcLabel.top, xOffset-rcLabel.left, rcLabel.Height());
				Gdiplus::SolidBrush brushFill( clrSel );

				Gdiplus::LinearGradientBrush lgb(rGPItem, clrSel, Gdiplus::Color(128, GetRValue(crHi), GetGValue(crHi), GetBValue(crHi)), 2);
				gfx.FillRectangle( &lgb, rGPItem );
			}

			// draw focus rectangle if necessary
			if (pNMCustomDraw->uItemState & CDIS_FOCUS)
			{
				CRect focus_rect(rcLabel.left, rcLabel.top, xOffset, rcLabel.bottom);
			...
			for (int i=1; i<nColsCnt; i++)
			{
				if (AfxExtractSubString(strSub, strText, i, '\t'))
				{
					...
					dc.DrawText(strSub, &rcText, DT_NOPREFIX | DT_END_ELLIPSIS | DT_RIGHT);
				}
				xOffset += m_arrColWidths[i];
			}
