...
IMPLEMENT_DYNCREATE(CCallerCalleeView, CColumnTreeView)

CCallerCalleeView::CCallerCalleeView()
{
...
BEGIN_MESSAGE_MAP(CCallerCalleeView, CColumnTreeView)
	ON_NOTIFY(TVN_SELCHANGED, TreeID, OnTvnSelchanged)
	ON_MESSAGE(WM_SETVIEWID, &CCallerCalleeView::OnSetViewID)
END_MESSAGE_MAP()


// CCallerCalleeView drawing

void CCallerCalleeView::OnDraw(CDC* pDC)
{
	CDocument* pDoc = GetDocument();
	// TODO: add draw code here
}

void CCallerCalleeView::OnTvnSelchanged(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CCallerCalleeView::OnInitialUpdate()
{
...
void CCallerCalleeView::AddFunctions(TreeNode* thisnode, HTREEITEM level)
{
	CTreeCtrl& tree = GetTreeCtrl();
	CString item;
...
LRESULT CCallerCalleeView::OnSetViewID(WPARAM wParam, LPARAM lParam)
{
	mythread = lParam;
...
#ifdef _DEBUG
void CCallerCalleeView::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CCallerCalleeView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif
...
CCodeProphetViewDoc* CCallerCalleeView::GetDocument() const // non-debug version is inline
{
